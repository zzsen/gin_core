...
```yaml
metrics:
  enabled: true                    # 是否启用 Prometheus 指标监控
  path: "/metrics"                 # 指标端点路径
...
tracing:
  ...
  exporterType: "otlp"             # 导出器类型：otlp / stdout
  endpoint: "localhost:4317"       # OTLP 采集器地址
  sampleRate: 1.0                  # 采样率：1.0=100%，生产环境建议 0.1-0.5
  insecure: true                   # 是否禁用 TLS
  propagatorType: "tracecontext"   # 传播格式：tracecontext / b3
  enableDBTracing: true            # 追踪数据库操作
  enableRedisTracing: true         # 追踪 Redis 操作
  enableHTTPClientTracing: true    # 追踪出站 HTTP 请求
```

> 详见 [链路追踪文档](./tracing.md)
...
rateLimit:
  ...
  message: "请求过于频繁"           # 默认限流提示
  cleanupInterval: 60              # 内存限流器清理间隔（秒）
  rules:                           # 自定义限流规则
    - path: "/api/login"
      rate: 5
      burst: 10
...
type BaseConfig struct {
    System       SystemInfo       `yaml:"system"`       // 系统功能开关配置
    Service      ServiceInfo      `yaml:"service"`      // HTTP 服务配置
    Log          LoggersConfig    `yaml:"log"`          // 日志配置
    ...
    RedisList    []RedisInfo      `yaml:"redisList"`    // 多 Redis 列表配置
    RabbitMQ     RabbitMQInfo     `yaml:"rabbitMQ"`     // RabbitMQ 配置
    RabbitMQList RabbitMqListInfo `yaml:"rabbitMQList"` // 多 RabbitMQ 列表配置
    Es           *EsInfo          `yaml:"es"`           // Elasticsearch 配置
    Smtp         SmtpInfo         `yaml:"smtp"`         // SMTP 邮件配置
}
```
