...
// 不需要 RabbitMQ 连接：仅验证空消息列表的快速返回逻辑
...
// 不需要 RabbitMQ 连接：设置基础配置仅为了让代码执行到 MQName 校验逻辑，
// 实际不会建立连接，因为在获取连接字符串时就会因找不到 MQName 而返回错误
...
// 不需要 RabbitMQ 连接：仅验证缓存命中逻辑，不涉及真实连接
