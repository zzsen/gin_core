...
type ServiceInfo struct {
	...
	TimeoutExcludePatterns []string `yaml:"timeoutExcludePatterns"` // 超时控制排除路径正则列表，匹配则跳过超时（SSE、WebSocket 等长连接）
...
// GetShutdownTimeout 获取优雅关闭超时时间（秒）
// 如果未配置或配置为 0，则返回默认值 5 秒
